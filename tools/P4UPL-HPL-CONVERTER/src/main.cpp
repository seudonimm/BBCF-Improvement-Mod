...
#define TITLE "P4UPL-HPL-CONVERTER (v1.00) - by GrimFlash\n\n"

void ConvertHplsToImpl()
{
	printf("HPL to P4UPL mode selected based on input files\n");
	printf("Converting HPLs to P4UPL\n\n");

	IMPL_t* loadedTemplate = LoadImplFromFile(L"*");
	if (!loadedTemplate)
	{
		printf("ERROR reading .p4upl file\n");
		return;
	}
	...
	{
		printf("ERROR writing .p4upl file\n");
	}
...
{
	printf("P4UPL to HPL mode selected based on input files\n");
	printf("Converting P4UPL to HPLs\n\n");
...
int main(int argc, char* argv[])
{
	...
	printf("Number of .p4upl files found: %d\n", implFiles);
	printf("Number of .hpl files found: %d\n\n", hplFiles);

	if (implFiles < 1)
	{
		printf("ERROR, there must be AT LEAST ONE .p4upl file present in the folder\n");
		ExitPrompt();
		return 2;
	}
	else if (implFiles > 1)
	{
		printf("ERROR, there must be ONLY ONE .p4upl file present in the folder\n");
		ExitPrompt();
		return 1;
	}
	...
	if (totalFiles == 1) //one file, can only be p4upl
	{
		ConvertImplToHpls();
	}
	else if (totalFiles >= 2) //two or more files (one p4upl and multiple hpls)
	{
		ConvertHplsToImpl();
	}
